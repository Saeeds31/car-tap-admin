                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in selectedCarList" :key="index">
                                <td>{{ index + 1 }}</td>
                                <td>{{ item.name ? item.name : item.label }}</td>
                                <td>
...
onMounted(async () => {
    try {
        const res = await axios.get(`/sales-plan/${route.params.id}`);
        saleplan.value = res.data.data;
        form.title = saleplan.value.title;
        form.start_date = saleplan.value.start_date;
        form.end_date = saleplan.value.end_date;
        initCars(res.data.data.cars)
    } catch (err) {
        console.log(err);
